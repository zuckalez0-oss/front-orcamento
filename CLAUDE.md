...
- **Nesting é retangular, não poligonal:** peças são tratadas pela bounding box `dimA x dimB` (mesmo quando importadas de DXF — o backend só extrai perímetro/área/bounding box do DXF, não guarda o contorno real). Nesting por contorno real (No-Fit-Polygon) foi avaliado e descartado por complexidade/tempo; ver `backend-orcamentos/nesting.py`.
...
Cada item de `detalhamento_espessuras` no retorno inclui, além dos totais de custo/tempo: `chapa_largura`, `chapa_comprimento`, `chapa_margem` (os valores efetivamente usados, já com fallback para 1200x3000/10mm se o frontend não mandou `configChapas` para aquela espessura) e `nesting: {chapas_necessarias, chapas: [{placements: [{id, x, y, width, height, rotated}]}]}` — coordenadas já absolutas na chapa real (mm), prontas para desenhar sem transformação adicional no frontend.

### Nesting (`backend-orcamentos/nesting.py`)

`nestear_pecas(pecas, largura_chapa, comprimento_chapa, margem, offset_peca)` — usa `rectpack` (`newPacker(rotation=True)`, multi-bin). Cada peça é expandida em `qtd` retângulos individuais `(dimA + offset_peca) x (dimB + offset_peca)`; o padding é subtraído de volta na posição final para que o espaçamento visual entre peças seja exatamente `offset_peca`. Levanta `NestingError` (capturada em `main.py` e convertida em `HTTPException 400`, nunca 500) quando: a margem é maior que a própria chapa, uma peça não cabe na área útil nem rotacionada, ou o total de instâncias de uma espessura passa de `LIMITE_INSTANCIAS` (2000 — trava de segurança para não travar a API síncrona com lotes gigantes). O frontend (`handleSalvar` em `App.jsx`) mostra o `detail` desse erro 400 direto num `alert()` e reabre o modal de configuração de chapa.
